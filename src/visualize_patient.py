...
def save_patient_report(
    ...
    ax4.axis("off")
    lines = [
        f"Patient: {case_id}   |   Model: {pred['variant']}   |   Slice {mid + 1}/{D}",
        "",
    ...
    fig.suptitle(f"EMIDEC Segmentation Report - {case_id}", fontsize=14, fontweight="bold")
...
def main():
    ...
    print(f"Model: {variant}  ({ckpt.name})  device={device}")
