...
Outputs under figures/paper/ and results/paper/:
  ...
  - SOTA comparison vs nnU-Net, ICPIU-Net, Schwab et al.
...
def plot_all_variants_overlay(histories: Dict[str, List[Dict]], out_dir: Path):
    ...
    ax.axhline(TARGET_MI_DICE, color="gray", ls="--", lw=1.5, label=f"ICPIU-Net target ({TARGET_MI_DICE})")
    ax.set_ylim(0, 1)
    ax.set_xlabel("Epoch")
...
def plot_sota_comparison(rows: List[Dict], out_dir: Path):
    ...
    ax.axhline(TARGET_MI_DICE, color="#E45756", ls="--", lw=1.2, alpha=0.7, label="ICPIU-Net MI target")
    ax.set_xticks(x)
    ax.set_xticklabels(methods)
    ax.set_ylabel("Dice Similarity Coefficient")
    ax.set_ylim(0, 1.05)
    ax.set_title(f"EMIDEC comparison - {MODEL_NAME} vs state-of-the-art")
    ...
    mi_vals = []
    for b in SOTA_BENCHMARKS:
        mi_methods.append(f"{b['method']} ({b['year']})")
        mi_vals.append(b["MI"])
    ...
    ax2.axvline(TARGET_MI_DICE, color="gray", ls="--", label=f"Target {TARGET_MI_DICE}")
    for b, v in zip(bars, mi_vals):
        ax2.text(v + 0.01, b.get_y() + b.get_height() / 2, f"{v:.3f}", va="center", fontsize=10)
    ax2.set_xlim(0, 1.0)
    ax2.set_xlabel("Myocardial Infarction Dice")
    ax2.set_title(f"MI segmentation - {MODEL_NAME} vs published EMIDEC results")
    ax2.legend()
    fig2.tight_layout()
...
def write_tables(rows: List[Dict], out_res: Path, split: str):
    ...
    fields = [
        ...
        "LV_dice", "MYO_dice", "MI_dice", "MVO_dice",
    ...
    with sota_csv.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["Method", "Citation", "Year", "Architecture", "LV Dice", "MYO Dice", "MI Dice", "MVO Dice", "Dataset"])
        for b in SOTA_BENCHMARKS:
            w.writerow([
                b["method"], b["citation"], b["year"], b["architecture"],
                b["LV"], b["MYO"], b["MI"], b["MVO"], b["dataset"],
            ])
    ...
    lines = [
        ...
        "",
        f"**Dataset:** EMIDEC (100 cases; stratified train/val/test)",
        f"**Split evaluated:** `{split}`",
        f"**Primary target:** MI Dice > {TARGET_MI_DICE} (ICPIU-Net, Brahim et al., 2022)",
        "",
    ...
    for r in rows:
        ...
        lines.append(
            ...
            f"{fmt(r['MI_dice'])} | {fmt(r['MVO_dice'])} | {fmt(r['params_M']) if r.get('params_M') else '-'} | "
    ...
    for b in SOTA_BENCHMARKS:
        def fmt(x):
            return f"{x:.3f}" if isinstance(x, float) else "-"
        lines.append(f"| {b['method']} ({b['citation']}) | {b['year']} | {fmt(b['LV'])} | {fmt(b['MYO'])} | {fmt(b['MI'])} |")
    if rows:
        ours = next((r for r in rows if r["variant"] == "M5"), rows[-1])
        lines.append(
            f"| **{MODEL_NAME} (this work)** | {MODEL_YEAR} | "
            f"**{ours['LV_dice']:.3f}** | **{ours['MYO_dice']:.3f}** | **{ours['MI_dice']:.3f}** |"
            if ours.get("MI_dice") is not None
            else f"| **{MODEL_NAME} (this work)** | {MODEL_YEAR} | - | - | - |"
        )
    lines += [
        "",
...
def render_table_figure(rows: List[Dict], out_dir: Path):
    ...
    ax2.axis("off")
    scol = ["Method", "Year", "LV Dice", "MYO Dice", "MI Dice"]
    scell = []
    for b in SOTA_BENCHMARKS:
        def fmt(x):
            return f"{x:.3f}" if isinstance(x, float) else "-"
        scell.append([b["method"], str(b["year"]), fmt(b["LV"]), fmt(b["MYO"]), fmt(b["MI"])])
    ours = next((r for r in rows if r["variant"] == "M5"), rows[-1] if rows else None)
    if ours:
        scell.append([
            MODEL_NAME,
            str(MODEL_YEAR),
            f"{ours['LV_dice']:.3f}" if ours.get("LV_dice") is not None else "-",
    ...
    t2.set_fontsize(11)
    t2.scale(1, 1.5)
    ax2.set_title(f"EMIDEC SOTA comparison including {MODEL_NAME}", fontweight="bold", pad=20)
